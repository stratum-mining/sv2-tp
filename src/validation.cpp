...
// Local standard headers used directly in this TU
#include <vector>
#include <optional>
#include <chrono>
#include <string>
...
// Define MEMPOOL_HEIGHT locally when not including txmempool.h
#ifndef MEMPOOL_HEIGHT
static const uint32_t MEMPOOL_HEIGHT = 0x7FFFFFFF;
#endif

namespace {
// Mempool acceptance logic removed in this build
} // namespace
...
// Mempool tracepoints removed in this build
...
// LimitMempoolSize omitted in this build (no mempool usage)
...
void Chainstate::MaybeUpdateMempoolForReorg(
    ...
    bool /*fAddToMempool*/)
{
    // No-op: mempool is not used in this build
}
...
* */
// Mempool-aware input checking omitted in this build.

// Mempool acceptance and package processing fully removed in this build

// (No MemPoolAccept in this build)

// AcceptToMemoryPool omitted in this build

// ProcessNewPackage omitted in this build
...
Chainstate::Chainstate(
    // CTxMemPool* mempool, // Not used in this build
    BlockManager& blockman,
    ChainstateManager& chainman,
    std::optional<uint256> from_snapshot_blockhash)
    // : m_mempool(mempool), // Not used in this build
      m_blockman(blockman),
      m_chainman(chainman),
...
{
    AssertLockHeld(::cs_main);
    const int64_t nMempoolUsage = 0; // mempool disabled
...
bool Chainstate::DisconnectTip(BlockValidationState& state, DisconnectedBlockTransactions* disconnectpool)
{
    ...
    // disconnectpool/mempool reorg handling omitted
    ...
    DisconnectedBlockTransactions& disconnectpool)
{
    ...
    // Mempool conflict removal omitted
...
bool Chainstate::ActivateBestChainStep(BlockValidationState& state, CBlockIndex* pindexMostWork, const std::shared_ptr<const CBlock>& pblock, bool& fInvalidFound, ConnectTrace& connectTrace)
{
        ...
        for (CBlockIndex* pindexConnect : vpindexToConnect | std::views::reverse) {
            if (!ConnectTip(state, pindexConnect, pindexConnect == pindexMostWork ? pblock : std::shared_ptr<const CBlock>(), connectTrace, disconnectpool)) {
                if (state.IsInvalid()) {
                    ...
                    // Failure while trying to connect
                    return false;
                }
            } else {
    ...
    // Reorg mempool update/check omitted
...
bool Chainstate::ActivateBestChain(BlockValidationState& state, std::shared_ptr<const CBlock> pblock)
{
    ...
    do {
...

        {
            LOCK(cs_main);
            {
            // Mempool is not used; no mempool lock required
